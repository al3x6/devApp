...
if (!empty($_POST['login']) and !empty($_POST['email']) and !empty($_POST['mdp']) and !empty($_POST['mdp_confirme']) and !empty($_POST['captcha'])) {
    ...
    if ($capt == $result) { //Vérification du bon résultat de la multiplication de la captcha
        if (strlen($login) < 30) { //Vérification de la taille du login
            ...
            if ($loginExist == 0) { //Vérification si l'utilisateur n'existe pas
                if (filter_var($email, FILTER_VALIDATE_EMAIL)) { //Vérification si l'adresse email est valide
                    $reqmail = $db->prepare("SELECT * FROM utilisateur WHERE email = ?");
                    $reqmail->execute(array($email));
                    $mailexist = $reqmail->rowCount();
                    if ($mailexist == 0) { //Vérification si l'utilisateur n'existe pas
